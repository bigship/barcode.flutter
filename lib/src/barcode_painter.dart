...
class BarCodePainter extends CustomPainter {
  BarCodePainter(this.data, this.type, this.lineWidth, this.hasText, this.foregroundColor, this.backgroundColor,
      {this.onError});
  ...
  void _drawBarCode93(Canvas canvas, Size size) {
    ...
    int sumC = 0, sumK = 0;
    bool hasError = false;
    ByteData strValue = ByteData(data.length);
    final painter = Paint()..style = PaintingStyle.fill;
    double height = hasText ? size.height * 0.85 : size.height;

    for (int i = 0; i < 8; i++) {
      Rect rect = Rect.fromLTWH(i * lineWidth, 0.0, lineWidth, height);
      painter.color = ((0x80 & (binSet[47] << i)) == 0x80) ? this.foregroundColor : this.backgroundColor;
      canvas.drawRect(rect, painter);
    }
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        Rect rect = Rect.fromLTWH(9 * lineWidth + 9 * i * lineWidth + j * lineWidth, 0.0, lineWidth, height);
    ...
    checkCodeC = sumC % 47;
    for (int i = 0; i < 8; i++) {
      Rect rect = Rect.fromLTWH(9 * lineWidth + (data.length * 9 + i) * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 8; i++) {
      Rect rect = Rect.fromLTWH(9 * lineWidth + ((data.length + 1) * 9 + i) * lineWidth, 0.0, lineWidth, height);
    ...
    //
    //  This one is the stop character '*'.
    //  Afterwards, there is a stop bar of one lineWidth
    //  that is painted with the foreground color
    //  To position it easily, we keep track of the current position
    //  incrementally.
    double left;
    for (int i = 0; i < 8; i++) {
      left = 9 * lineWidth + ((data.length + 2) * 9 + i) * lineWidth;
      Rect rect = Rect.fromLTWH(left, 0.0, lineWidth, height);
      //Rect rect = Rect.fromLTWH(9 * lineWidth + ((data.length + 2) * 9 + i) * lineWidth, 0.0, lineWidth, height);
    ...
    canvas.drawRect(rect, painter);
    left += lineWidth;
    rect = Rect.fromLTWH(left, 0.0, lineWidth, height);
    painter.color = this.foregroundColor;
    canvas.drawRect(rect, painter);
    //
    //  This next part appears to be wrong according to wikipedia and the
    //  barcode generators I tried around the web.
    //  It has been replaced with a space and stop bar in the code above.
    //
    // for (int i = 0; i < 8; i++) {
    //   Rect rect = Rect.fromLTWH(9 * lineWidth + ((data.length + 3) * 9 + i) * lineWidth, 0.0, lineWidth, height);
    //   ((0x80 & (binSet[1] << i)) == 0x80) ? painter.color = this.foregroundColor : painter.color = this.backgroundColor;
    //   canvas.drawRect(rect, painter);
    // }
    ...
    if (hasText) {
      for (int i = 0; i < data.length; i++) {
        ...
        TextPainter textPainter = TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
        textPainter.layout();
        textPainter.paint(canvas, Offset((size.width - data.length * 8 * lineWidth) / 2 + 8 * i * lineWidth, height));
      }
    }
  }
  ...
  void _drawBarCode128(Canvas canvas, Size size) {
    ...
    ByteData strValue = ByteData(strlen);
    ...
    for (int i = 0; i < strlen; i++) {
      ...
      for (int j = 0; j < 11; j++) {
        Rect rect = Rect.fromLTWH(11 * lineWidth + 11 * i * lineWidth + j * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 11; i++) {
      Rect rect = Rect.fromLTWH(11 * lineWidth + (strlen * 11 + i) * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 13; i++) {
      Rect rect = Rect.fromLTWH(22 * lineWidth + (strlen * 11 + i) * lineWidth, 0.0, lineWidth, height);
  ...
  void _drawBarCodeEAN13(Canvas canvas, Size size) {
    ...
    for (int j = 0; j < 3; j++) {
      Rect rect = Rect.fromLTWH(11 * lineWidth + j * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (i == 0) {
        ...
        if ((0x20 & (flagCode[flagbit] << (i - 1))) == 0) {
          for (int j = 0; j < 7; j++) {
            Rect rect = Rect.fromLTWH(14 * lineWidth + 7 * (i - 1) * lineWidth + j * lineWidth, 0.0, lineWidth, height);
          ...
          for (int n = 0; n < 7; n++) {
            Rect rect = Rect.fromLTWH(14 * lineWidth + 7 * (i - 1) * lineWidth + n * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 5; i++) {
      Rect rect = Rect.fromLTWH(56 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        Rect rect = Rect.fromLTWH(61 * lineWidth + j * lineWidth + 7 * i * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 7; i++) {
      Rect rect = Rect.fromLTWH(96 * lineWidth + i * lineWidth, 0.0, lineWidth, height);
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(103 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ...
      for (int i = 0; i < strlen; i++) {
        if (i == 0) {
          ...
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(13 * lineWidth + 7 * (i - 1) * lineWidth, height));
        } else if (i == 12) {
          TextSpan span =
              TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: checkCode.toString());
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(17 * lineWidth + 7 * (i - 1) * lineWidth, height));
        } else {
          TextSpan span = TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: data[i]);
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
  ...
  void _drawBarCodeEAN8(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(7 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
    ...
    for (int i = 0; i < 4; i++) {
      tmpCode = data.codeUnitAt(i) - 48;
      for (int j = 0; j < 7; j++) {
        Rect rect = Rect.fromLTWH(10 * lineWidth + j * lineWidth + 7 * i * lineWidth, 0.0, lineWidth, height);
        ((0x40 & (codeA[tmpCode] << j)) == 0x40)
            ? painter.color = this.foregroundColor
            : painter.color = this.backgroundColor;
        canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 5; i++) {
      Rect rect = Rect.fromLTWH(38 * lineWidth + i * lineWidth, 0.0, lineWidth, height);
      ((0x01 & (midCodeSep >> i)) == 0x01)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
    ...
    for (int i = 4; i < 7; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        Rect rect = Rect.fromLTWH(43 * lineWidth + j * lineWidth + 7 * (i - 4) * lineWidth, 0.0, lineWidth, height);
        ((0x40 & (codeC[tmpBarCode] << j)) == 0x40)
            ? painter.color = this.foregroundColor
            : painter.color = this.backgroundColor;
        canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 7; i++) {
      Rect rect = Rect.fromLTWH(64 * lineWidth + i * lineWidth, 0.0, lineWidth, height);
      ((0x40 & (codeC[checkCode] << i)) == 0x40)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(71 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ((0x01 & (endCodeSep >> i)) == 0x01)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
      ...
      for (int i = 0; i < strlen; i++) {
        if (i < 4) {
          ...
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(11 * lineWidth + 7 * i * lineWidth, height));
        } else if (i == 7) {
          TextSpan span =
              TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: checkCode.toString());
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(15 * lineWidth + 7 * i * lineWidth, height));
        } else {
          TextSpan span = TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: data[i]);
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
  ...
  void _drawBarCodeUPCA(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(9 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ((0x01 & (startCodeSep >> i)) == 0x01)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 6; i++) {
      tmpCode = data.codeUnitAt(i) - 48;
      for (int j = 0; j < 7; j++) {
        Rect rect = Rect.fromLTWH(12 * lineWidth + 7 * i * lineWidth + j * lineWidth, 0.0, lineWidth, height);
        ((0x40 & (codeA[tmpCode] << j)) == 0x40)
            ? painter.color = this.foregroundColor
            : painter.color = this.backgroundColor;
        canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 5; i++) {
      Rect rect = Rect.fromLTWH(54 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ((0x01 & (midCodeSep >> i)) == 0x01)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        Rect rect = Rect.fromLTWH(59 * lineWidth + j * lineWidth + 7 * i * lineWidth, 0.0, lineWidth, height);
        ((0x40 & (codeC[tmpBarCode] << j)) == 0x40)
            ? painter.color = this.foregroundColor
            : painter.color = this.backgroundColor;
        canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 7; i++) {
      Rect rect = Rect.fromLTWH(94 * lineWidth + i * lineWidth, 0.0, lineWidth, height);
      ((0x40 & (codeC[checkCode] << i)) == 0x40)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(101 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ((0x01 & (endCodeSep >> i)) == 0x01)
          ? painter.color = this.foregroundColor
          : painter.color = this.backgroundColor;
      canvas.drawRect(rect, painter);
      ...
      for (int i = 0; i < strlen; i++) {
        if (i == 0) {
          ...
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(16 * lineWidth + 7 * (i - 1) * lineWidth, height));
        } else if (i < 11) {
          TextSpan span = TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: data[i]);
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(26 * lineWidth + 7 * (i - 1) * lineWidth, height));
        } else {
          TextSpan span =
              TextSpan(style: TextStyle(color: this.foregroundColor, fontSize: 15.0), text: checkCode.toString());
          TextPainter textPainter =
              TextPainter(text: span, textAlign: TextAlign.left, textDirection: TextDirection.ltr);
          textPainter.layout();
          textPainter.paint(canvas, Offset(35 * lineWidth + 7 * (i - 1) * lineWidth, height));
        }
      }
    }
  }
  ...
  void _drawBarCodeUPCE(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      Rect rect = Rect.fromLTWH(8 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if ((0x20 & (checkCodeFlag[checkCode] << i)) == 0x20) {
        for (int j = 0; j < 7; j++) {
          Rect rect = Rect.fromLTWH(11 * lineWidth + 7 * i * lineWidth + j * lineWidth, 0.0, lineWidth, height);
        ...
        for (int k = 0; k < 7; k++) {
          Rect rect = Rect.fromLTWH(11 * lineWidth + 7 * i * lineWidth + k * lineWidth, 0.0, lineWidth, height);
          painter.color = ((0x40 & (codeA[tmpCode] << k)) == 0x40) ? this.foregroundColor : this.backgroundColor;
          canvas.drawRect(rect, painter);
        }
      }
    }
    ...
    for (int i = 0; i < 6; i++) {
      Rect rect = Rect.fromLTWH(53 * lineWidth + i * lineWidth, 0.0, lineWidth, hasText ? height * 1.08 : height);
      ...
      for (int i = 0; i < strlen; i++) {
        if (i == 0) {
          TextSpan span = TextSpan(
            style: TextStyle(color: this.foregroundColor, fontSize: 15.0),
            text: data[i],
          );
